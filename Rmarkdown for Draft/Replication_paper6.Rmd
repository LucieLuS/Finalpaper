...
```{r bivariate relationship, eval = TRUE, echo = FALSE, results = "hide", message = FALSE}
...
```{r bivariate relationship, eval = TRUE, echo = FALSE, results='hide', message = FALSE, warning = FALSE, include=FALSE}
...
```{r bivariate relationship, results = "asis", message = FALSE, warning = FALSE}
...
```{r compare trueATE and the other types of ATE,  results='hide', eval=FALSE, echo=TRUE, cache=TRUE}

trueATE #36.71
##sampdistmeans<-apply(estdists,1,mean)
#meanATE
apply(estdists[1:3,],1,mean) #sampledistmeans: very close to the trueATE


#absolute difference between the estimated mean and the true ATE
abs(trueATE-apply(estdists[1:3,],1,mean)) # abs(): absoluate positive values: the differences between trueATE and the estimates
...
apply(estdists[1:3,],1,function(x){ mean( ( x - trueATE)^2 ) })
```
...
```{r try randomization and p-value for lm, results='hide', echo = FALSE, message = FALSE, warning=FALSE, include=F}
...
mean(expBeta_lm5Bs)
plot(density(expBeta_lm5Bs)) #plot the null distribution
...
```{r try randomization and p-value for lmrob, results='hide', echo = FALSE, message = FALSE, warning=FALSE, include=F}
...
#One-sided p-value
pvalue_lm <- as.numeric(as.matrix(summary(expBeta_lmrob_5Bs < simlmrobtest_5Bs)))/length(expBeta_lm5Bs)
pvalue_lm #0.051
...
#If we run it a couple of times, the false positive rates are slightly different, but they are around 0.05. It seems to be valid test, which actually come close to the nominal false positive rate.

#How to interpret this: you create a null effect knowing that your null hypothesis is true. Then the false positive rate 0.09 means out of 100 times, the test faslely reject the nulls (knowing the null is true but you still reject it).
...
```{r showing valid test for lmrob}
err_ratetlmrob<- function(y){
    require(robustbase)
    shuffledz <- sample(mydata4_small$Z) 
  sim_psrob <- summary(lmrob(y ~ shuffledz+ log(GDP_l1_s) + EC_l1_s + WTO_l1_s, data=mydata4_small))$coef[2,4]
...
M_test <- mean(results_lmrob5Bs <= .05)#0.043
M_test
SD_test <- sd(results_lmrob5Bs <= .05)
SD_test

#Calculate power formally by power analysis
power<-pwr.t.test(d=(M_test-0.05)/SD_test, n=length(mydata4_small$Z),sig.level=0.05,type="one.sample",alternative="two.sided")$power 
power #0.1437

abs((M_test-0.05))/SD_test #0.03449

#Calculate Cohen's d.
op <- par(mar = c(5,7,4,4)) #change white-space around graph
hist(results_lmrob5Bs, breaks=20, xlab="P-values", ylab="number of p-values\n", axes=T,
     main=paste("P-value distribution under null effect in the simulation"),
...
#When there is no true effect, p-values are what is called 'uniformly distributed under the null'. Every p-value is equally likely when the null hypothesis is true, and every bar in the graph will contain 5% of all the p-values (as indicated by the dotted red line).
```




#Cohen describes effect size as "the degree to which the null hypothesis is false."This is a crucial part of using the pwr package correctly: You must provide an effect size on the expected scale. Doing otherwise will produce wrong sample size and power calculations.
...
powerfn <- function(simulations,trt,outcome,block,effect) {
  ...
  output<-foreach(1:simulations,
    ...
    newoutcome <- outcome - newz*effect #the outcome without the hypothesized effect
...
power_fun<- function(simulation, outcome, effect)
  {
    #require(robustbase)
    shuffledz <- sample(mydata4_small$Z) 
    ## since we know that there is no effect, then we can assess the false positive rate of lm
    newoutcome <- outcome - shuffledz*effect
    sim_power<- summary(lm(newoutcome ~ shuffledz+ log(GDP_l1_s) + EC_l1_s + WTO_l1_s, data=mydata4_small))$coef[2,4]
  return(sim_power)
#return(output)
}
...
#set.seed(12345)
somepower <- sapply(c(.1,.5,.8,1,3,5,10), function(effectSize){
                      res<-power_fun(1000,outcome = mydata4_small$Tariff, effect=effectSize)
                    power <- (mean(res < .05))
                      return(power)
                  })
...
#https://stats.stackexchange.com/questions/74057/assessing-the-power-of-a-normality-test-in-r
```

```{r, results='hide', echo = FALSE, message = FALSE, warning=FALSE, include=F}
