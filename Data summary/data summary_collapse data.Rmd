...
```{r bivariate relationship print, eval = TRUE, echo = FALSE, results='hide', message = FALSE, warning = FALSE, include=FALSE}
...
```{r bivariate relationship print2, results = "asis", message = FALSE, warning = FALSE}
...
acrosscountries<-var(countryMeans)
withincountries<-mean(with(mydata4_small,tapply(Tariff,countryF,var)), na.rm = T)
...
acrosscountries #123.7
withincountries #70.71
...
withincountries/(acrosscountries+withincountries) #0.36
acrosscountries/(acrosscountries+withincountries) #0.63

#install.packages("ICC")
library(ICC)
ICC_wc <- ICCbare(y=Tariff,x=countryF,data=mydata4_small) #0.63
ICC_wc
...
library(rmngb)
boxplot(mydata4_small$Tariff ~ mydata4_small$countryF)
## to visualize variances accross countries

boxplot(mydata4_small$Tariff ~ mydata4_small$Year)
