...
```{r Confidence Interval using pvalue, results='hide', echo = FALSE, message = FALSE, warning=FALSE, include=F}
...
######
#library(foreach)
res1<-foreach(h=seq(-5,5,0.5),.combine='c') %dopar% {message("."); MyFisherTest4(x=h, thez=mydata4_small$Regime_l1_Bi)}
...
printCIres1 <- rbind(seq(-5,5,0.5),res1)
printCIres1
...
CI_m1 <- round(quantile(beta.obs.boot,c(.025,.975)), 2)
#[-1.90, 3.11]
...
CI_m2 <- round(quantile(resample_original[,"Regime_l1_Bi_s"],c(.025,.975)),2)
